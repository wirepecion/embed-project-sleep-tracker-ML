...
COLLECTION_SCORES = "interval_records"  # New target for interval scores
...
def set_diffuser_state(is_on: bool):
    ...
    # Safe format in case token is missing
    if not BLYNK_AUTH_TOKEN: 
        logger.warning("Blynk Token missing. Skipping diffuser control.")
        return
...
def process_active_sessions():
    ...
    # OPTIMIZATION: Only scan last 24h to prevent infinite growth
...
def process_single_session_intervals(session_id: str):
    ...
    if should_open_diffuser:
        logger.info(f"📉 Low Score (<{DIFFUSER_THRESHOLD_SCORE}%). Activating Diffuser...")
...
def process_finished_sessions():
    ...
    # OPTIMIZATION: Only check sessions ended in last 20 mins
    ...
    try:
        ...
        for session in ended_sessions:
            try:
                ...
                # FLUSH: Process any pending readings before summarizing
                process_single_session_intervals(session.id)
...
def generate_session_summary(session_id: str, target_ref):
    ...
    # 1. Fetch Session Doc (For Duration)
